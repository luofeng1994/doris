...
parser grammar FLDorisParser;

options { tokenVocab = FLDorisLexer; }
...
// -----------------Expression-----------------
namedExpression
    : expression (AS? identifierOrText)? (SELECT_FIELD_COMMENT)?
    ;

namedExpressionSeq
    : namedExpression (COMMA (SELECT_FIELD_COMMENT)? namedExpression)*
    ;
